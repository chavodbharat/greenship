...
const SearchFilter = ({route}: any) => {
        ...
        if (res) {
          ...
          if (Object.keys(obj).length > 0) {
            ...
            allData = allData.map((str, index) => ({
              title: str.substring(0, 1).toUpperCase() + str.substring(1),
              id: str,
            }));
  ...
  const dropDownPosition = (position: number) => {
    ...
    if (position == 0) {
      setAnimalState(prev => ({
        ...prev,
        actionSheetData: animalState.petArtListData,
        isActionSheetShow: true,
      }));
    } else if (position == 1) {
      setAnimalState(prev => ({
        ...prev,
        actionSheetData: animalState.petRaceListData,
        isActionSheetShow: true,
      }));
    } else if (position == 2) {
      setAnimalState(prev => ({
  ...
  const dropDownUserPosition = (position: number) => {
    ...
    if (position == 0) {
      setState(prev => ({
        ...prev,
        actionSheetData: state.allMemberTypeData,
        isActionSheetShow: true,
      }));
    }
  };
  ...
  const clickOnActionSheetUserOption = async (index: number) => {
    ...
    if (actionSheetPosition == 0) {
      setState(prev => ({
        ...prev,
        selectedMemberType: allMemberTypeData[index].title,
        isActionSheetShow: false,
      }));
    }
  };
  ...
  const onSearchPress = () => {
    if (state.activeTab == 0) {
      ...
      navigate(SEARCH_PET_USER_LIST_SCREEN.name, {
        isUser: true,
        name,
        memberType:
          selectedMemberType === 'Member Type' ? '' : selectedMemberType,
      ...
      navigate(SEARCH_PET_USER_LIST_SCREEN.name, {
        ...
        radius: selectedRadius,
        profilePic: state.userProfilePic,
        petArt: selectedPetArt,
        petRace: selectedPetRace,
        petAge: selectedAge,
      });
    }
  };
  ...
  return (
    // TODO: Which header color set in this screen.
    <View style={{flex: 1, backgroundColor: colors.listBackGradientThree}}>
      <SafeAreaView style={[styles.main, {backgroundColor: 'transparent'}]}>
        <Header statusBarColor={darkColors.communityGreenColor} />

        <ScrollView
          style={{backgroundColor: colors.defaultViewBackgroundColor}}
            ...
            <View style={styles.boxView}>
              ...
              {state.activeTab == 0 ? (
                <View style={{width: '100%', paddingVertical: scale(10)}}>
                  ...
                  <Pressable onPress={() => dropDownUserPosition(0)}>
                    <View
                      style={[
                      ...
                      <View style={styles.flexOne}>
                        <Text
                          style={[
                            ...
                            state.selectedMemberType != 'Member Type' && {
                              color: colors.black,
                            },
                          ]}>
                ...
                <View style={{width: '100%', paddingVertical: scale(10)}}>
                    ...
                    <View
                      ...
                      <View style={styles.flexOne}>
                        <Text
                          style={[
                            styles.dropdownLabelStyle,
                            animalState.selectedPetArt != 'Art' && {
                              color: colors.black,
                            },
                          ]}>
                          {animalState.selectedPetArt}
                        </Text>
                      </View>
                      <View style={styles.flexZero}>
                        <Image
                    ...
                    <View
                      ...
                      <View style={styles.flexOne}>
                        <Text
                          style={[
                            styles.dropdownLabelStyle,
                            animalState.selectedPetRace != 'Race' && {
                              color: colors.black,
                            },
                          ]}>
                          {animalState.selectedPetRace}
                        </Text>
                      </View>
                      <View style={styles.flexZero}>
                        <Image
                  ...
                  <Pressable onPress={() => dropDownPosition(2)}>
                    <View
                      style={[
                        styles.textInputCustomStyle,
                        {flexDirection: 'row'},
                      ...
                      <View style={styles.flexOne}>
                        <Text
                          style={[
                            styles.dropdownLabelStyle,
                            animalState.selectedGender != 'Gender' && {
                              color: colors.black,
                            },
                          ]}>
                          {animalState.selectedGender}
                        </Text>
                      </View>
                      <View style={styles.flexZero}>
                        <Image
                  ...
                  <View style={{margin: scale(20)}}>
                    <Text
                      style={[styles.radiusTxtStyle, {marginTop: scale(20)}]}>
                      Radius
                    </Text>
                    <CustomRadiusSeekbar
                      dataArray={['25 km', '50 km', '100 km', '200 km']}
        ...
        <ActionSheetModal
          ...
          <ActionSheet
            ...
            onCancelPress={() =>
              setAnimalState(prev => ({...prev, isActionSheetShow: false}))
            }
