...
const CommunityUserList = ({route}: any) => {
      ...
      getSearchUserList(body, (res: any) => {
        if (res) {
          ...
          setState(prev => ({
            ...prev,
            loader: false,
            communityListData: data.members_list,
          }));
        } else {
  ...
  const renderItem = ({item, index}: any) => {
    return (
      <LinearGradient
        ...
        colors={
          index % 2 == 0
            ? [colors.communityGreenColor, colors.lightGreen]
            : [colors.filterListOne, colors.filterListTwo]
        }
        style={[
          styles.mainView,
          index % 2 != 0 && {borderWidth: 1, borderColor: colors.lightGreen},
        ]}>
        <Pressable onPress={() => redirectToUserDetails(item.ID)}>
          <View style={styles.memberViewParentView}>
            ...
            <View
              style={[
                styles.flexOne,
                {marginLeft: scale(15), alignSelf: 'center'},
              ]}>
              <Text
                style={[
                  styles.memberListItemTextValueStyle,
                  {
              ...
              <Text
                style={[
                  styles.memberListItemDesTextValueStyle,
                  {
                    color: index % 2 == 0 ? colors.white : colors.black,
                  },
                ]}>
  ...
  return (
    <LinearGradient
      style={{flex: 1}}
      colors={[colors.communityGreenColor, colors.defaultViewBackgroundColor]}>
