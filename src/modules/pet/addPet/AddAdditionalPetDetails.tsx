...
const AddAdditionalPetDetails = ({route}: any) => {
  ...
  const setPetDetails = () => {
    if (isEdit && Object.keys(petObj).length > 0) {
      setState(prev => ({
        ...prev,
        petAge: petObj.pet_age,
        petSize: petObj.pet_size,
        petWeight: petObj.pet_weight,
        petColors: petObj.pet_color,
        selectedFamilyTree: petObj.pet_stammbaum,
  ...
  const onPublishPress = () => {
    ...
    if (
      ...
      petWeight &&
      petColors &&
      selectedPetMissing != missingDefaultMessage &&
      ...
      if (isEdit) {
        try {
          Promise.all([createNewPet(), uploadPetProfileImage()]).then(
            function () {
        ...
        try {
          Promise.all([
            ...
            uploadPetProfileMultipleImages,
          ]).then(function () {
            setState(prev => ({...prev, loader: false}));
            showMessage({
      ...
      if (!petAge) {
        ...
        showMessage({
          message: 'Please enter pet colors with comma separate',
          type: 'danger',
        });
  ...
  const uploadPetProfileImage = () =>
    new Promise(function (resolve, reject) {
      if (petProfilePicRes && !petProfilePicRes.isEditMode) {
        const formData = new FormData();
        formData.append('pet_profile', {
          name: petProfilePicRes.path.substring(
            petProfilePicRes.path.lastIndexOf('/') + 1,
          ),
          type: petProfilePicRes.mime,
          uri:
            Platform.OS === 'ios'
              ? petProfilePicRes.path.replace('file://', '')
  ...
  const openGallery = () => {
    try {
      ImagePicker.openPicker({
        ...
        setState(prev => ({
          ...prev,
          allMultipleImageLocalArray: data,
          imageModalVisible: !prev.imageModalVisible,
        }));
      });
    } catch (e) {
  ...
  const uploadPetProfileMultipleImages = () =>
    new Promise(function (resolve, reject) {
      if (state.allMultipleImageLocalArray) {
        const formData = new FormData();
        state.allMultipleImageLocalArray.forEach(
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView
        ...
        <View style={[styles.flexOne, {padding: scale(25)}]}>
          ...
          {petProfilePicRes && (
            <DigitalerTierpassView petProfilePicRes={petProfilePicRes} />
          )}
          <TextInput
            mode="outlined"
            value={state.petAge}
          ...
          <TextInput
            ...
            onChangeText={weight =>
              setState(prev => ({...prev, petWeight: weight}))
            }
          />
          <TextInput
            mode="outlined"
            ...
            style={styles.textInputStyle}
            onChangeText={color =>
              setState(prev => ({...prev, petColors: color}))
            }
              ...
              <View style={styles.flexOne}>
                <Text
                  style={[
                    ...
                    state.selectedFamilyTree != 'Family tree' && {
                      color: colors.black,
                    },
                  ]}>
                  {state.selectedFamilyTree}
                </Text>
              </View>
            ...
            <View style={[styles.textInputCustomStyle, {flexDirection: 'row'}]}>
              <View style={styles.flexOne}>
                <Text
                  style={[
                    styles.dropdownLabelStyle,
                    state.selectedPetMissing != 'Missing' && {
                      color: colors.black,
                    },
                  ]}>
                  {state.selectedPetMissing}
                </Text>
              </View>
              <View style={styles.flexZero}>
                <Image
          ...
          <View style={styles.checkBoxParentViewBack}>
            <Checkbox
              ...
              onPress={() =>
                setState(prev => ({
                  ...prev,
                  isTermsConditionsCheck: !state.isTermsConditionsCheck,
                }))
              }
            />
            <Text style={styles.petNameTextStyle}>
              {'Yes, I agree to the  '}
              <TouchableWithoutFeedback
                onPress={() => {
                  Linking.openURL(
              ...
              {'  of use of Greensheep'}
            </Text>
          </View>
          <Button
            labelStyle={styles.loginFontStyle}
            style={[styles.allButonStyle, {marginBottom: verticalScale(50)}]}
            mode="contained"
