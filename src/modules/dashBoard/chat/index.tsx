...
const ChatList = () => {
      ...
      const promises = fields.map(async (obj: any, index: number) => {
        if(Object.keys(obj).length > 0) {
          try {
            ...
            let channelName = 'Channel_' + [userData?.id, finalId].sort().join('_');
  ...
  const renderFriendItem = ({item, index}: any) => {
   ...
   // console.log("JSON", JSON.stringify(item));
    ...
    const strChannelsObj = item.channelStatus.channels['Channel_' + [userData?.id, finalId].sort().join('_')];
    let userStatus = 0;
    if(Object.keys(strChannelsObj).length > 0){
      const strOccupants = strChannelsObj.occupants;
      let users = strOccupants.find((data: any) => data.uuid.toString() === finalId.toString());
        ...
        <Pressable
          ...
          <View style={[styles.flexDirectionRowView,{marginTop: verticalScale(10),
            ...
            <View style={styles.flexZeroView}>
              ...
              <View style={[styles.onlineStatusView, userStatus == 1 && {backgroundColor: 
