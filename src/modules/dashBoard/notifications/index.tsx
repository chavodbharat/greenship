...
const Notifications = () => {
        ...
        if (Object.keys(obj).length > 0) {
          try {
            const response = await fetch(
              `${serviceUrl.apiUrl}buddypress/v1/members/${obj.item_id}`,
            );
  ...
  const renderItem = ({item, index}: any) => {
    ...
    return item.action === 'friendship_request' ? (
      <View style={styles.notificationParentView}>
        ...
        <View style={styles.notificationTitleParentView}>
          <Text style={styles.notificationTitleStyle}>
            {item.name + ' sent you friend request'}
          </Text>
          <View style={styles.notificationActionParentView}>
            ...
            <View
              ...
              <Pressable
                style={[
                  styles.btnStyle,
                  {backgroundColor: colors.emergencyGradientOne},
                ]}
      ...
      <View style={styles.notificationParentView}>
        ...
        <View style={styles.notificationTitleParentView}>
          <Text style={styles.notificationTitleStyle}>
            {'You accepted friend request of ' + item.name}
          </Text>
          <Text style={styles.notificationDateStyle}>{days}</Text>
        </View>
      </View>
      ...
      <View style={styles.notificationParentView}>
        <View style={styles.notificationImageParentView}>
          <Image
            style={styles.notificationImageView}
            source={
              item.image ? {uri: item.image} : AllImages.appPlaceholderIcon
            }
          />
        </View>
        <View style={styles.notificationTitleParentView}>
